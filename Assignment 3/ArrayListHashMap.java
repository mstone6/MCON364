...
import Assignment3.MapEntry;

import java.util.ArrayList;
import java.util.Iterator;
import java.util.List;

public class ArrayListHashMap<K, V> implements MapInterface<K, V> {
    //Now creating a list of all lists to store all the key value pairs
    protected List<List<MapEntry<K, V>>> buckets;
    ...
    public ArrayListHashMap(int initCapacity) {
    ...
    public V put(K k, V v) {
        ...
        List<MapEntry<K, V>> bucket = buckets.get(bucketIndex);
    ...
    public V remove(K k) { // Removes the entry associated with the specified key
        ...
        List<MapEntry<K, V>> bucket = buckets.get(bucketIndex); // Gets the bucket corresponding to the index

        Iterator<MapEntry<K, V>> iterator = bucket.iterator(); // Initializes iterator for the bucket
    ...
    public boolean contains(K k) { // Checks if the map contains the specified key
        ...
        for (MapEntry<K, V> entry : bucket) { // Iterates through entries in the bucket
    ...
    public boolean isEmpty() { // Checks if the map is empty
        for (List<MapEntry<K, V>> bucket : buckets) { // Iterates through all buckets
    ...
    public Iterator<MapEntry<K, V>> iterator() {
        ...
        List<MapEntry<K, V>> entries = new ArrayList<>(); // Initializes a list to store all entries
        for (List<MapEntry<K, V>> bucket : buckets) {
            // Now Iterating through all buckets
    ...
    private int hash(K key) {
        // Chooses between naive or sophisticated hash function
        return sophisticatedHash(key.toString()); // Uses sophisticated hash function by default
    }
